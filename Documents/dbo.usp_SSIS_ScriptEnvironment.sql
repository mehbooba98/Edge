...
AS
        ...
        */
		...
		--[MA-20181125] Following line change as per tutorial
       -- SELECT [env_var] = @tab + @tab + ', @' + ev.name + ' '  + ev.base_data_type  + '= N''' + ISNULL(CONVERT(varchar(max), ev.value), '<REPLACE_ME>') + ''''
	    SELECT [env_var] = @tab + @tab + ', @' + ev.name + ' '  + iif(ev.base_data_type='nvarchar', 'nvarchar(4000), ev.base_data_type)  + '= N''' + ISNULL(CONVERT(varchar(max), ev.value), '<REPLACE_ME>') + ''''
                        , [name] = ev.name
        INTO #env_var
